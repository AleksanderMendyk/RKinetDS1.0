...
ui <- shiny::fluidPage(
  ...
  navbarPage("RKinetDS",
             ...
             navbarMenu("Results", icon = icon("glyphicon glyphicon-stats", lib = "glyphicon", id = "overall_tab"),
                        ...
                        tabPanel("Error ranking",  tableOutput("table_error")),
...
server <- function(input, output, session) {
  ...
  observeEvent(input$run, {
    ...
    if (!chosen()) {
      ...
      #Show error ranking
      file_error <- file.path("error_ranking.txt")
      output$table_error <- renderTable({
      read.table(file_error, header=TRUE, fill = TRUE)
        }, hover=TRUE)
        ...
        # Read the best data from error_ranking.txt
        error_table_txt <- read.csv("error_ranking.txt", header = TRUE, sep="\t")
